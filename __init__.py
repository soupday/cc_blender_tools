...
image_types = [".bmp", ".sgi", ".rgb", ".bw", ".png", ".jpg", ".jpeg", ".jp2", ".j2c", ".tga", ".cin", ".dpx", ".exr", ".hdr", ".tif", ".tiff"]
...
def get_ao_strength(object, material):
    ...
    if is_eye_material(material):
        ...
        return "tongue_specular", props.tongue_ao
...
def input_is_image_node(input, filename):    
    if len(input.links) == 1:        
        from_node = input.links[0].from_node        
        if from_node.type == "TEX_IMAGE":
            image = from_node.image
            if image.type == "IMAGE" and image.filepath != "":
                if os.path.samefile(image.filepath, filename):
                    return True
    return False
        ...
        if (i.type == "IMAGE" and i.filepath != ""):
            try:
                if os.path.normcase(i.filepath) == os.path.normcase(filename):
                    print("    Using existing image: " + i.filepath)
                    return i
            except:
                pass
...
def is_image_opaque(image):
    ...
    # dirty hack to determine if opacity texture has none opaque pixels
    # copy the image and scale it to 8x8
    # any alpha pixels *should* reduce the opacity in the scaled copy
    copy = image.copy()
    copy.scale(8,8)
...
##
def find_material_texture(material, suffix_list):
...
def set_node_input(node, socket, value):
    if node is not None:
        try:    
            ...
            print("Unable to set input: " + node.name + "[" + str(socket) + "]")
...
def set_node_output(node, socket, value):
    if node is not None:
        try:
            ...
            print("Unable to set output: " + node.name + "[" + str(socket) + "]")
...
def link_nodes(links, from_node, from_socket, to_node, to_socket):
    if from_node is not None and to_node is not None:
        try:    
            ...
            print("Unable to link: " + from_node.name + "[" + str(from_socket) + "] to " + to_node.name + "[" + str(to_socket) + "]")
...
def connect_basic_eye_material(object, material, shader):
    ...
    #
    reset_cursor()
    diffuse_file = find_material_texture(material, base_color_map)       
    ...
    #
    reset_cursor()
    normal_file = find_material_texture(material, sclera_normal_map)    
...
def connect_adv_eye_material(object, material, shader):
    ...
    # maps
    diffuse_file = find_material_texture(material, base_color_map)
    sclera_file = find_material_texture(material, sclera_map)
    blend_file = find_material_texture(material, mod_basecolorblend_map)
    ao_file = find_material_texture(material, mod_ao_map)
    advance_cursor(4 - count_maps(diffuse_file, sclera_file, blend_file, ao_file))
    if diffuse_file is not None:
        advance_cursor(-1)
        drop_cursor(0.75)
    ...
    set_node_input(sss_node, "Radius1", props.eye_sss_radius * subsurface_scale)
    set_node_input(sss_node, "Radius2", props.eye_sss_radius * subsurface_scale)
    ...
    advance_cursor(-1)
    # maps
    snormal_file = find_material_texture(material, sclera_normal_map)
    if snormal_file is None:
        advance_cursor()
    if snormal_file is not None:
        advance_cursor(-1)
        drop_cursor(0.75)
        group = get_node_group("tiling_mapping")
        sclera_tiling_node = make_node_group_node(nodes, group, "Sclera Normal Tiling", "tiling_normal_sclera_mapping")
        advance_cursor()
    ...
    if snormal_file is not None:
        ...
        link_nodes(links, sclera_tiling_node, "Vector", snormal_node, "Vector")
...
def connect_adv_mouth_material(object, material, shader):
    ...
    if teeth:
        mask_file = find_material_texture(material, teeth_gums_map)
    ...
    # just connect as default advanced material
    gradient_file = find_material_texture(material, teeth_gradient_map)
    ...
    roughness = 0
    if teeth:
        specular = props.teeth_specular
        roughness = props.teeth_roughness
    else:
        specular = props.tongue_specular
        roughness = props.tongue_roughness
    # maps
    metallic_file = find_material_texture(material, metallic_map)
    roughness_file = find_material_texture(material, roughness_map)    
...
def connect_basic_material(object, material, shader):    
    ...
    #
    reset_cursor()
    specular_file = find_material_texture(material, specular_map)
    mask_file = find_material_texture(material, mod_specmask_map)
    ...
    #
    reset_cursor()
    emission_file = find_material_texture(material, emission_map)
    ...
    #
    reset_cursor()
    alpha_file = find_material_texture(material, alpha_map)
    ...
    if is_really_opaque:
        set_material_alpha(material, "OPAQUE")                
    elif is_input_connected(shader.inputs["Alpha"]) or shader.inputs["Alpha"].default_value < 1.0:            
    ...
    #
    reset_cursor()
    normal_file = find_material_texture(material, normal_map)
    bump_file = find_material_texture(material, bump_map)
    if normal_file is not None:
        normal_node = make_image_node(nodes, normal_file, "normal_tex", "Non-Color")        
        advance_cursor()
        normalmap_node = make_shader_node(nodes, "ShaderNodeNormalMap", 0.6)
...
def connect_subsurface(object, material, shader, diffuse_node):
    ...
    sss_file = find_material_texture(material, subsurface_map)
    trans_file = find_material_texture(material, mod_transmission_map)    
...
def connect_msr(object, material, shader):
    ...
    prop_roughness = "skin_roughness_power"
    roughness_power = props.skin_roughness_power
    prop_group = get_material_group(object, material)
    ...
    if mask_file is None:
        advance_cursor()
    if specular_file is None:
        advance_cursor()
    if roughness_file is None:     
        advance_cursor()
    if metallic_file is None:     
        advance_cursor()
    # maps
    if roughness_file is not None:
        roughness_node = make_image_node(nodes, roughness_file, "roughness_tex", "Non-Color")        
    ...
    if "Roughness Power" in group_node.inputs:
...
def connect_emission_alpha(object, material, shader):
    ...
    if is_really_opaque:
        ...
        if is_hair_object(object, material) or is_eyelash_material(material):
            ...
            if props.blend_mode == "HASHED":
                set_material_alpha(material, "HASHED")
            elif props.blend_mode == "BLEND":
                set_material_alpha(material, "BLEND")
            else:
                set_material_alpha(material, "OPAQUE")
    else:
        set_material_alpha(material, "OPAQUE")
...
def connect_normal(object, material, shader):
    ...
    normal_file = find_material_texture(material, normal_map)
    bump_file = find_material_texture(material, bump_map)
    blend_file = find_material_texture(material, mod_normalblend_map)
    micro_file = find_material_texture(material, mod_micronormal_map)
    mask_file = find_material_texture(material, mod_micronormalmask_map)
    prop_bump, bump_strength = get_bump_strength(object, material)
...
node_groups = ["color_ao_mixer", "color_blend_ao_mixer", "color_eye_mixer", "color_teeth_mixer", "color_tongue_mixer", 
               ...
               "msr_mixer", "msr_overlay_mixer", "msr_skin_mixer", 
...
def process_material_slots(object):
    ...
    for slot in object.material_slots:
            print("Processing Material: " + slot.material.name)
...
def process_object(object):
    ...
    print("Processing Object: " + object.name + ", Type: " + object.type)    
...
def get_image_paths(filepath):
    ...
    if type == "fbx":
        ...
        print("FBX Import, looking for main image path:")
        
        if os.path.exists(fbm_dir):
            print("    Found image path: " + fbm_dir)
            paths.append(fbm_dir)
        
        print("FBX Import, scanning object materials for paths:")
        ...
        print("OBJ Import, looking for image path:")
        
        tex_dir = os.path.join(dir, name)
        if os.path.exists(tex_dir):
            print("    Found image path: " + tex_dir)
...
def add_spot_light(name, location, target, energy, blend, size, distance):
    target = add_target("Target_" + name, target)
...
class CC3Scene(bpy.types.Operator):
    ...
    def description(cls, context, properties):
        ...
        if properties.param == "DEFAULT_LIGHTS":
            return "Add the default 3 point lighting arrangement"
        elif properties.param == "CAMERA":
            return "Add camera"
        elif properties.param == "SETTINGS":
            return "Apply the settings for ambient occlusion, bloom and color mapping"
        return ""


def quick_set_process_object(param, object):
    global objects_processed  
    ...
    if object is not None and object not in objects_processed:
        ...
        for child in object.children:
            quick_set_process_object(param, child)
...
def quick_set_execute(param):     
    ...
    if param == "RESET":
        ...
        for p in props.import_objects:
            if p.object is not None:
                quick_set_process_object(param, p.object)
    else:
        for obj in bpy.context.selected_objects:
            quick_set_process_object(param, obj)
...
def set_node_from_property(node):
    ...
    #
        ...
        if "_skin_" in name:
            set_node_input(node, "Radius", props.skin_sss_radius * subsurface_scale)
            set_node_input(node, "Falloff", props.skin_sss_falloff)
        # subsurface_eye_mixer
        elif "_eye_" in name:
            set_node_input(node, "Radius1", props.eye_sss_radius * subsurface_scale)
            set_node_input(node, "Radius2", props.eye_sss_radius * subsurface_scale)
        ...
        # subsurface_hair_mixer
            ...
            set_node_input(node, "Radius", props.hair_sss_radius * subsurface_scale)
            set_node_input(node, "Falloff", props.hair_sss_falloff)
        # subsurface_teeth_mixer
        elif "_teeth_" in name:
            set_node_input(node, "Radius1", props.teeth_sss_radius * subsurface_scale)
            set_node_input(node, "Radius2", props.teeth_sss_radius * subsurface_scale)
        ...
        # subsurface_tongue_mixer
            ...
            set_node_input(node, "Radius", props.tongue_sss_radius * subsurface_scale)
            set_node_input(node, "Falloff", props.tongue_sss_falloff)
        # subsurface_nails_mixer
        elif "_nails_" in name:
            set_node_input(node, "Radius", props.nails_sss_radius * subsurface_scale)
            set_node_input(node, "Falloff", props.nails_sss_falloff)
        # subsurface_default_mixer
        elif "_default_" in name:
            set_node_input(node, "Radius", props.default_sss_radius * subsurface_scale)
    ...
    #
        ...
        if "_skin_" in name:
            set_node_input(node, "Roughness Power", props.skin_roughness_power)
...
class CC3ImportProps(bpy.types.PropertyGroup):
    ...
    skin_basic_specular: bpy.props.FloatProperty(default=0.2, min=0, max=2, update=quick_set_update)
    ...
    skin_roughness_power: bpy.props.FloatProperty(default=0.8, min=0.5, max=1.5, update=quick_set_update)
    skin_specular: bpy.props.FloatProperty(default=0.3, min=0, max=2, update=quick_set_update)
...
class MyPanel(bpy.types.Panel):
    ...
    def draw(self, context):        
        ...
        if fake_drop_down(layout.box().row(), 
            ...
            if props.setup_mode == "ADVANCED":               
                ...
                if fake_drop_down(layout.row(), 
                    ...
                    col_1.label(text="*Skin Roughness Power")
                ...
                if fake_drop_down(layout.row(), 
                    ...
                    col_1.label(text="Eye SSS Radius")
                    col_2.prop(props, "eye_sss_radius", text="", slider=True)
                    col_1.label(text="Eye SSS Faloff")
                ...
                if fake_drop_down(layout.row(), 
                    ...
                    col_1.label(text="Default SSS Radius")
                    col_2.prop(props, "default_sss_radius", text="", slider=True)
                    col_1.label(text="Default SSS Falloff")
                    col_2.prop(props, "default_sss_falloff", text="")
                    col_1.label(text="Default Micro Normal")
                    col_2.prop(props, "default_micronormal", text="", slider=True)
                    col_1.label(text="Default MNormal Tiling")
        ...
        if fake_drop_down(layout.box().row(), 
            ...
            if props.update_mode == "UPDATE_ALL":
                op.param = "FETCH_ALL"
            elif props.update_mode == "UPDATE_SELECTED":
                op.param = "FETCH_SELECTED"
...
class MyPanel3(bpy.types.Panel):
    ...
    def draw(self, context):        
        ...
        col_1.label(text="Default Lights")
        op = col_2.operator("cc3.scene", icon="LIGHT", text="Lights")
        op.param = "DEFAULT_LIGHTS"
        
        col_1.label(text="Portrait Lights")
        op = col_2.operator("cc3.scene", icon="LIGHT", text="Lights")
        op.param = "PORTRAIT_LIGHTS"
        
        col_1.label(text="Render Settings")
        op = col_2.operator("cc3.scene", icon="TOOL_SETTINGS", text="Settings")
        op.param = "SETTINGS"
        
        col_1.label(text="Add Camera")
        op = col_2.operator("cc3.scene", icon="CAMERA_DATA", text="Camera")
        op.param = "CAMERA"
