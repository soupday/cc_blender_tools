...
def prep_bake(mat = None, samples = BAKE_SAMPLES, image_format = IMAGE_FORMAT, make_surface = True):
    ...
    if utils.is_blender_version("3.0.0"):
        ...
        bake_state["use_denoising"] = bpy.context.scene.cycles.use_denoising
        bake_state["use_preview_denoising"] = bpy.context.scene.cycles.use_preview_denoising
        bake_state["use_auto_tile"] =  bpy.context.scene.cycles.use_auto_tile
    # render settings
    bake_state["file_format"] = bpy.context.scene.render.image_settings.file_format
    bake_state["color_depth"] = bpy.context.scene.render.image_settings.color_depth
    bake_state["color_mode"] = bpy.context.scene.render.image_settings.color_mode
    bake_state["use_bake_multires"] =bpy.context.scene.render.use_bake_multires
    bake_state["use_selected_to_active"] = bpy.context.scene.render.bake.use_selected_to_active
    bake_state["use_pass_direct"] = bpy.context.scene.render.bake.use_pass_direct
    bake_state["use_pass_indirect"] = bpy.context.scene.render.bake.use_pass_indirect
    ...
    bake_state["look"] = bpy.context.scene.view_settings.look
    bake_state["gamma"] = bpy.context.scene.view_settings.gamma
    bake_state["exposure"] = bpy.context.scene.view_settings.exposure
    bake_state["colorspace"] = bpy.context.scene.sequencer_colorspace_settings.name
    ...
    bake_state["cycles_bake_type"] = bpy.context.scene.cycles.bake_type
    bake_state["render_bake_type"] = bpy.context.scene.render.bake_type
...
def get_bake_image(mat, channel_id, width, height, shader_node, socket, bake_dir,
    ...
    is_data = True
    alpha = False
    rgb_sockets = [ "Diffuse", "Base Color", "Emission", "Subsurface Color" ]
...
def prep_diffuse(mat, shader_node):
    ...
    if shader_node:
        nodeutils.set_node_input_value(shader_node, "AO Strength", props.ao_in_diffuse)
...
def bake_export_material(mat, source_mat, source_mat_cache):
    ...
    utils.log_info("Baking export material " + props.target_mode + " / " + mat.name)
    utils.log_info("")
    ...
    ao_bake_node = None
    ao_strength = 1.0
    if nodeutils.has_connected_input(bsdf_node, "Base Color"):
    ...
    sss_bake_node = None
    sss_radius = 1.0
    if nodeutils.has_connected_input(bsdf_node, "Subsurface"):
    ...
    if nodeutils.has_connected_input(bsdf_node, "Metallic"):
    ...
    if nodeutils.has_connected_input(bsdf_node, "Specular"):
    ...
    nodeutils.set_node_input_value(bsdf_node, "Specular", 0.5 * specular_scale)

    # Roughness Maps
    roughnesss_bake_node = None
    if nodeutils.has_connected_input(bsdf_node, "Roughness"):
    ...
    if nodeutils.has_connected_input(bsdf_node, "Emission"):
    ...
    if nodeutils.has_connected_input(bsdf_node, "Alpha") or (shader_node and "Opacity" in shader_node.outputs):
    ...
    bump_bake_node = None
    bump_distance = 0.01
    if nodeutils.has_connected_input(bsdf_node, "Normal"):
    ...
    reconnect_material(mat, source_mat_cache, ao_strength, sss_radius, bump_distance, normal_strength, micro_normal_strength, micro_normal_scale)
...
def combine_diffuse_tex(nodes, source_mat, source_mat_cache, mat, diffuse_node, alpha_node):

    diffuse_data = None
    alpha_data = None
    bsdf_node = nodeutils.get_bsdf_node(mat)
    diffuse_value = nodeutils.get_node_input_value(bsdf_node, "Base Color", (1,1,1,1))
    ...
    image, exists = get_image_target(image_name, width, height, path, is_data=False, has_alpha=True, channel_packed=True)
    image_node = nodeutils.make_image_node(nodes, image, image_node_name)
    image_node.select = True
    nodes.active = image_node
...
def combine_hdrp_mask_tex(nodes, source_mat, source_mat_cache, mat,
    ...
    bsdf_node = nodeutils.get_bsdf_node(mat)
    metallic_value = nodeutils.get_node_input_value(bsdf_node, "Metallic", 0.0)
    roughness_value = nodeutils.get_node_input_value(bsdf_node, "Roughness", 0.0)
    ao_value = 1
    mask_value = 1

    image_name = get_bake_image_name(mat, map_suffix)
    image_node_name = get_bake_image_node_name(mat, map_suffix)
    image, exists = get_image_target(image_name, width, height, path, is_data=True, has_alpa=True, channel_packed=True)
    image_node = nodeutils.make_image_node(nodes, image, image_node_name)
    image_node.select = True
    nodes.active = image_node
    image_data = list(image.pixels)
...
def combine_hdrp_detail_tex(nodes, source_mat, source_mat_cache, mat,
    ...
    image_name = get_bake_image_name(mat, map_suffix)
    image_node_name = get_bake_image_node_name(mat, map_suffix)
    image, exists = get_image_target(image_name, width, height, path, is_data=True, has_alpha=True, channel_packed=True)
    image_node = nodeutils.make_image_node(nodes, image, image_node_name)
    image_node.select = True
    nodes.active = image_node
    image_data = list(image.pixels)
...
def make_metallic_smoothness_tex(nodes, source_mat, source_mat_cache, mat,
    ...
    bsdf_node = nodeutils.get_bsdf_node(mat)
    roughness_value = nodeutils.get_node_input_value(bsdf_node, "Roughness", 0.5)
    metallic_value = nodeutils.get_node_input_value(bsdf_node, "Metallic", 0)
...
def combine_gltf(nodes, source_mat, source_mat_cache, mat,
    ...
    image_name = get_bake_image_name(mat, map_suffix)
    image_node_name = get_bake_image_node_name(mat, map_suffix)
    image, exists = get_image_target(image_name, width, height, path, is_data=True, has_alpha=False, channel_packed=False)
    image_node = nodeutils.make_image_node(nodes, image, image_node_name)
    image_node.select = True
    nodes.active = image_node
    image_data = list(image.pixels)
...
def reconnect_material(mat, mat_cache, ao_strength, sss_radius, bump_distance, normal_strength, micro_normal_strength, micro_normal_scale):
    ...
    if props.target_mode == "GLTF" and props.pack_gltf:
        ...
        if tex_nodes["BaseMap"]:
            nodeutils.link_nodes(links, tex_nodes["BaseMap"], "Color", bsdf_node, "Base Color")
            if tex_nodes["Alpha"]:
                nodeutils.link_nodes(links, tex_nodes["BaseMap"], "Alpha", bsdf_node, "Alpha")
        ...
        if tex_nodes["GLTF"]:
            ...
            if utils.is_blender_version("3.3.0"):
                ...
                nodeutils.link_nodes(links, split_node, "Green", bsdf_node, "Roughness")
                nodeutils.link_nodes(links, split_node, "Blue", bsdf_node, "Metallic")
            else:
    ...
    if tex_nodes["Diffuse"] or tex_nodes["AO"]:
        ...
        if props.target_mode == "GLTF": # unpacked GLTF diffuse & AO
            nodeutils.link_nodes(links, tex_nodes["Diffuse"], "Color", bsdf_node, "Base Color")
            if not gltf_settings_node:
                gltf_settings_node = nodeutils.make_gltf_settings_node(nodes)
    ...
    if tex_nodes["Subsurface"]:
        nodeutils.link_nodes(links, tex_nodes["Subsurface"], "Color", bsdf_node, "Subsurface")
    ...
    if tex_nodes["Metallic"]:
        nodeutils.link_nodes(links, tex_nodes["Metallic"], "Color", bsdf_node, "Metallic")
    if tex_nodes["Specular"]:
        nodeutils.link_nodes(links, tex_nodes["Specular"], "Color", bsdf_node, "Specular")
    if tex_nodes["Roughness"]:
        nodeutils.link_nodes(links, tex_nodes["Roughness"], "Color", bsdf_node, "Roughness")
    if tex_nodes["Emission"]:
        nodeutils.link_nodes(links, tex_nodes["Emission"], "Color", bsdf_node, "Emission")
    if tex_nodes["Alpha"]:
        nodeutils.link_nodes(links, tex_nodes["Alpha"], "Color", bsdf_node, "Alpha")
    if tex_nodes["Transmission"]:
        nodeutils.link_nodes(links, tex_nodes["Transmission"], "Color", bsdf_node, "Transmission")
...
def bake_character(chr_cache):
    ...
    for obj in objects:
        if obj.type == "MESH":
            bake_character_object(chr_cache, obj, bake_state, materials_done)
        elif obj.type == "ARMATURE":
            for child in obj.children:
                bake_character_object(chr_cache, child, bake_state, materials_done)
...
def bake_character_object(chr_cache, obj, bake_state, materials_done):
    ...
    for slot in obj.material_slots:
        ...
        if source_mat not in materials_done:
            ...
            #try:
            bake_export_material(bake_mat, source_mat, source_mat_cache)
